...
public class Cenario7 {
	...
	@Test
	@DisplayName("Usuario padrao pede 5 exemplares livres para 10 dias. "
			+ " Devolve o primeiro o livro 5 dias depois ")
	void teste() throws Exception {
		...
		DadosEmprestimo pedidoEmprestimo3 = new DadosEmprestimo(idLivro3,
				usuario1.idUsuario, 10, TipoExemplar.LIVRE, 3);
		DadosEmprestimo pedidoEmprestimo4 = new DadosEmprestimo(idLivro4,
				usuario1.idUsuario, 10, TipoExemplar.LIVRE, 4);
		DadosEmprestimo pedidoEmprestimo5 = new DadosEmprestimo(idLivro5,
				usuario1.idUsuario, 10, TipoExemplar.LIVRE, 5);
		...
		Assertions.assertTrue(resultados.stream()
				.filter(r -> r.getMomentoDevolucao().isPresent()).count() == 1);
